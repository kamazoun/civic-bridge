...
const HELP = {
  en: {
    published_notice: "Everything here is a notice an office published on the Public Information Portal — exactly what the office wrote, not checked by anyone else, with the office named. Use the subject buttons to narrow the list.",
    ...
    everything: "Every notice published for your area, newest first, by any office at any level. Filter by subject with the buttons.",
    ...
    records_on_file: "Notices this office has published on the portal.",
    ...
    votes: "What residents make of this notice: two independent votes (Helpful / Needs clarity), public comments — optionally 'speaking as' a trader, a parent, a herder — and the questions sent to the office.",
  ...
  fr: {
    published_notice: "Tout ce qui est ici est un avis publié par un bureau sur le portail d’information publique — exactement ce que le bureau a écrit, vérifié par personne d’autre, avec le bureau nommé. Les boutons de sujet permettent de filtrer.",
    ...
    everything: "Tous les avis publiés pour votre zone, du plus récent au plus ancien, par tous les bureaux à tous les niveaux. Filtrez par sujet avec les boutons.",
    ...
    records_on_file: "Les avis que ce bureau a publiés sur le portail.",
    ...
    votes: "Ce que les résidents pensent de cet avis : deux votes indépendants (Utile / Manque de clarté), des commentaires publics — éventuellement « au nom de » commerçant·e, parent, éleveur·se — et les questions envoyées au bureau.",
...
const SPEAKING_AS = ["trader", "farmer", "herder", "parent", "student", "health", "transport", "elder", "business", "youth"];
...
const STRINGS = {
  en: {
    ...
    filter_all_issues: "All subjects",
    ...
    stat_card_commitments: "Notices published",
    stat_card_commitments_detail: "by this office",
    stat_card_questions_detail: "from residents, on its notices",
    ...
    perspective_label: "Speaking as (optional)",
    speaking_as_label: "Speaking as (optional)",
    speaking_as_none: "Myself",
    voice_trader: "a market trader", voice_farmer: "a farmer", voice_herder: "a herder", voice_parent: "a parent", voice_student: "a student", voice_health: "a health worker", voice_transport: "a transport operator", voice_elder: "an elderly resident", voice_business: "a business owner", voice_youth: "a young resident",
    no_voices_yet: "No one has spoken for a group yet.",
    ...
    track_record_entry_commitments: (n, date) => `${n} notice${n === 1 ? "" : "s"} published · last activity: ${date}.`,
  ...
  fr: {
    ...
    filter_all_issues: "Tous les sujets",
    ...
    stat_card_commitments: "Avis publiés",
    stat_card_commitments_detail: "par ce bureau",
    stat_card_questions_detail: "de résidents, sur ses avis",
    ...
    perspective_label: "Au nom de (facultatif)",
    speaking_as_label: "Au nom de (facultatif)",
    speaking_as_none: "Moi-même",
    voice_trader: "un·e commerçant·e du marché", voice_farmer: "un·e agriculteur·rice", voice_herder: "un·e éleveur·se", voice_parent: "un parent d’élève", voice_student: "un·e élève / étudiant·e", voice_health: "un·e agent·e de santé", voice_transport: "un·e transporteur·rice", voice_elder: "un·e aîné·e du quartier", voice_business: "un·e chef·fe d’entreprise", voice_youth: "un·e jeune du quartier",
    no_voices_yet: "Personne n’a encore parlé au nom d’un groupe.",
    ...
    track_record_entry_commitments: (n, date) => `${n} avis publié${n === 1 ? "" : "s"} · dernière activité : ${date}.`,
...
function applyAreaContext() {
  ...
  // Everything shown is a published notice; the built-in example records only
  // serve as a fallback on a server that has nothing published yet.
  if (relevantNotices.length) { state.records = []; state.issues = []; state.news = []; }
  ...
  if (state.dashboard.metrics[0]) state.dashboard.metrics[0] = { ...state.dashboard.metrics[0], label: isFrench ? "Avis publiés" : "Published notices", value: String(state.issues.length), detail: isFrench ? "Par les bureaux de votre pays" : "By the offices in your country" };
  if (state.dashboard.metrics[2]) state.dashboard.metrics[2] = { ...state.dashboard.metrics[2], label: isFrench ? "Sujets" : "Subjects", value: String(new Set(state.issues.map((issue) => issue.type)).size), detail: isFrench ? "Eau, marchés, santé, écoles…" : "Water, markets, health, schools…" };
  ...
  app.innerHTML = `
    ...
    <section class="section-grid rep-detail-grid">
      <article class="panel"><span class="eyebrow">${t("group_posts_eyebrow")} ${help("group_conversation")}</span><h2>${t("group_posts_title")}</h2>
        ...
        <div class="comment-list">${group.posts.length ? group.posts.map((item) => `<article class="comment-item"><div class="comment-item-top"><strong>${esc(item.user_label)}${item.speaking_as ? ` <span class="tag">${esc(item.speaking_as)}</span>` : ""}</strong><small>${esc(formatActivityDate(item.created_at))}</small></div><p>${esc(item.message)}</p></article>`).join("") : `<div class="empty-state">${t("group_no_posts")}</div>`}</div>
      </article>
      <aside class="panel"><span class="eyebrow">${t("group_records_eyebrow")} ${help("group_records")}</span><h2>${t("group_records_title")}</h2><p class="panel-intro">${t("group_records_body")}</p>
        ${related.length ? related.map((issue) => `<button class="watch-row plain" data-record="${esc(issue.id)}"><span><strong>${esc(issue.title)}</strong><small>${esc(issue.type)} · ${esc(issue.last_update)}</small></span><span class="watch-arrow">↗</span></button>`).join("") : `<div class="empty-state">${t("group_no_records")}</div>`}
...
function allRecordsSection() {
  const subjects = [...new Set(state.issues.map((issue) => issue.type).filter(Boolean))].sort((a, b) => a.localeCompare(b));
  const filters = ["All issues", ...subjects];
  const filterLabels = Object.fromEntries(filters.map((filter) => [filter, filter === "All issues" ? t("filter_all_issues") : filter]));
  const filterHelp = { "All issues": help("published_notice") };
  const visible = state.issueFilter === "All issues" || !subjects.includes(state.issueFilter) ? state.issues : state.issues.filter((issue) => issue.type === state.issueFilter);
  return `
...
function renderOverview(record) {
  ...
  return `
    ...
    <div class="section-grid" style="margin-top:18px"><article class="panel"><span class="eyebrow">${t("community_view_eyebrow")} ${help("votes")}</span><h2>${t("community_view_title")}</h2><p class="panel-intro">${t("community_view_body")}</p><div class="columns-3"><div class="mini-column"><h4>${t("shared_facts")}</h4><p>${esc(record.facts[0] || "")}</p></div><div class="mini-column"><h4>${t("open_questions")}</h4><p>${esc(record.unknowns[0] || "")}</p></div><div class="mini-column"><h4>${t("different_priorities_col")}</h4><p>${esc([...new Set((state.community[record.id]?.comments || []).map((item) => item.speaking_as).filter(Boolean))].slice(0, 4).join(" · ") || t("no_voices_yet"))}</p></div></div></article><aside class="panel"><span class="eyebrow">${t("next_step_eyebrow")} ${help("next_step")}</span><h2>${t("next_step_title")}</h2><p class="panel-intro">${t("next_step_body")}</p><p class="addressed-to">${t("addressed_to", esc(responsibleOffice(record).name))}</p><button class="primary-btn" data-tab="feedback">${t("draft_feedback_arrow")}</button>${(() => { const topic = issueTopic(record); const group = state.groups.find((item) => item.topic === topic); const names = TOPIC_NAMES[uiLang()] || TOPIC_NAMES.en; return topic ? `<div class="lands-on"><span class="eyebrow">${t("lands_on_eyebrow")} ${help("lands_on")}</span><p>${group ? t("lands_on_body", group.members, esc(names[topic] || topic), esc(state.dashboard.area)) : t("lands_on_body_unknown", esc(names[topic] || topic))}</p><button class="text-btn" data-open-group="${topic}">${t("open_group_arrow")}</button></div>` : ""; })()}</aside></div>
  ...
  return `
    <div class="feedback-layout">
      <article class="panel"><span class="eyebrow">${t("your_words_first_eyebrow")} ${help("feedback_tab")}</span><h2>${t("your_words_first_title")}</h2><p class="panel-intro">${t("your_words_first_body")}</p>
        ...
        <label class="form-label" for="perspective">${t("perspective_label")} ${help("perspective")}</label><select id="perspective" class="feedback-select"><option>${t("community_question")}</option>${SPEAKING_AS.map((key) => `<option>${esc(t(`voice_${key}`))}</option>`).join("")}</select>
...
function renderFeedbackList() {
  ...
  const myFeedback = state.user ? state.feedback.filter((item) => !item.country || item.country === state.dashboard.country) : [];
...
function bindRecordActions(record) {
  ...
  if (commentButton) commentButton.addEventListener("click", () => requireUser(async () => {
    ...
    if (!message) return showToast(t("toast_comment_required"));
    const response = await fetch(`/api/records/${encodeURIComponent(record.id)}/comments`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token: authToken(), country: state.dashboard.country, message, speaking_as: document.querySelector("#comment-as")?.value || "", anonymous: Boolean(document.querySelector("#comment-anonymous")?.checked), persona: anonymousPersona() }) });
    const data = await response.json();
    if (!response.ok) return showToast(data.error || t("toast_comment_failed"));
